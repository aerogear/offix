import { useEffect, useReducer, useCallback, Dispatch, useMemo } from "react";
  ...
  useMemo(() => {
    ...
    try {
      ...
      if ((typeof selector) === "string") {
        results = model.queryById(selector as string);
      } else {
        results = model.query(selector);
      }
      results.then((data) => {
        dispatch({ type: ActionType.REQUEST_COMPLETE, data: data });
      }).catch((error) => {
